...
# پرداختی باید از تیتر کم شود، نه فقط در پانوشت بیاید
check("تیتر فاکتور وقتی پرداختی هست مانده را نشان می‌دهد",
      "money(_bal if _paid else t[" in APP and "_bal = int(t.get" in APP,
      "واسطه‌ای که نصف حسابش را داده بود، بالای فاکتور همان عدد اول را "
      "می‌دید و فکر می‌کرد پرداختش ثبت نشده")
check("و زیرش می‌نویسد کل چقدر بود و چقدرش داده شده",
      "پرداخت‌شده" in APP and "_paid:" in APP)
check("مانده هنوز due منهای paid است", '"balance": due - paid' in APP)
