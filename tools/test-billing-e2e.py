...
# تاریخِ خراب فاکتور را نمی‌ترکاند و به پیش‌فرضِ گروه برمی‌گردد — ولی از
# ۱.۱۰۳ بی‌صدا نه: برچسبِ منبع می‌گوید کدام تاریخ نامعتبر بود. پیش‌تر همین
# پرشِ بی‌صدا «تسویه‌شده تا»ی خراب را هم می‌بلعید و ماه‌های تسویه‌شده
# دوباره روی فاکتور می‌آمدند.
_bw = APP.billing_invoice("بازه", start="۱۴۰۴/۰۱/۰۱", x_admin_password="x")["sinceWhy"]
check("تاریخ خراب فاکتور را نمی‌ترکاند و به پیش‌فرض برمی‌گردد",
      _bw.startswith("شروع همکاری"), _bw)
check("و گفته می‌شود که تاریخِ انتخابی نامعتبر بود", "نامعتبر" in _bw, _bw)
