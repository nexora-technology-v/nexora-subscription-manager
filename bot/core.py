...
def trial_within_cap(plan, cap):
    """
    آیا تستِ نماینده از تستِ خودِ مالک بزرگ‌تر نیست؟

    برمی‌گرداند: (مجاز؟, دلیل به فارسی)

    چرا سقف: تستِ نماینده رایگان است و هزینه‌اش با مالک (تصمیمِ
    مالک). بدونِ سقف، هر پلنی با برچسبِ «تست» کانفیگِ رایگانِ بی‌حد
    می‌شد. سقف از تستِ **خودِ مالک** می‌آید — عددی حدس زده نمی‌شود.

    صفر در سقف یعنی «نامحدود» (همان قراردادِ پلن)؛ صفر در تستِ
    نماینده وقتی سقف محدود است، یعنی بزرگ‌تر از سقف.

    هم ذخیره‌ی پلن (بکند) و هم ساختِ کانفیگ (ربات) از همین می‌خوانند:
    اگر مالک بعداً تستش را کوچک کند، تستِ قدیمیِ نماینده دیگر ساخته
    نمی‌شود.
    """
    if not cap:
        return False, "مالک هنوز تست رایگانِ فعالی ندارد، پس تستِ نماینده هم ممکن نیست"
    for key, label, unit in (("gb", "حجم", "گیگ"), ("days", "مدت", "روز"),
                             ("ip_limit", "تعداد کاربر", "کاربر")):
        lim = int(cap.get(key) or 0)
        val = int((plan or {}).get(key) or 0)
        if lim > 0 and (val <= 0 or val > lim):
            return False, f"{label}ِ تست حداکثر {lim} {unit} است"
    return True, ""
