...
RUNSRC = io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
...
RUN2 = io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "run.py"), encoding="utf-8").read()
...
check("و حتی در شکست واقعی، سفارشِ تحویل‌شده را برنمی‌گرداند",
      RUN2.count("AND sub_id IS NULL") >= 2,
